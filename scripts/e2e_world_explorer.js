...
const OUT = '/tmp/bunnies-shots/10-world-explorer.png';
...
async function playLevel(page, level) {
    await page.evaluate((lv) => {
        ...
        window.game.scene.getScenes(true).forEach((s) => { s._navTx = false; });
        const sc = window.game.scene.getScenes(true)[0];
        if (typeof NavSystem !== 'undefined') NavSystem.ready(sc);
        NavSystem.go(sc, 'WorldExplorerScreen', { gameId: 'world_explorer', level: lv });
    }, level);
    await page.waitForFunction(() => window.game.scene.isActive('WorldExplorerScreen'), { timeout: 15000 });
    await page.evaluate(() => {
        const g = window.game.scene.getScene('WorldExplorerScreen');
    ...
    const rounds = await page.evaluate(() => window.game.scene.getScene('WorldExplorerScreen').levelCfg.rounds);
    for (let round = 0; round < rounds; round++) {
        await page.waitForFunction(() => {
            const g = window.game.scene.getScene('WorldExplorerScreen');
            return g && g.acceptingInput && g.country;
        }, { timeout: 8000 });
        const last = round === rounds - 1;
        const t0 = last ? Date.now() : 0;
        await page.evaluate(() => {
            window.game.scene.getScene('WorldExplorerScreen').autoSolveRound();
        });
        if (last) {
...
(async () => {
    ...
    const hasCity = await page.evaluate(() => {
        const m = window.game.scene.getScene('MenuScreen');
        return (m.cityMarkers || []).some((c) => c.getData('city') && c.getData('city').screenKey === 'WorldExplorerScreen');
    });
    if (!hasCity) throw new Error('world explorer city missing on menu');
    ...
    if (!/Bunny Khám Phá Thế Giới/.test(texts)) throw new Error('missing game name');
    ...
    console.log('PASS world explorer e2e', OUT);
    await browser.close();
