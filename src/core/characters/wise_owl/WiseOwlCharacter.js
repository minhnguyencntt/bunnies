...
class WiseOwlCharacter {
     ...
     * Uses a SINGLE unified movement system to prevent teleportation
     */
    startRandomMovements() {
        if (!this.sprite) return;
        
        // Stop any existing movements first
        this.stopMovements();
        ...
        const margin = this.size / 2 + 20;
        const minX = margin;
        const maxX = sceneWidth - margin;
        const minY = margin;
        const maxY = sceneHeight - margin;
        
        // Store base position and offsets for smooth layered movement
        this.baseX = this.sprite.x;
        this.baseY = this.sprite.y;
        this.floatOffset = 0;
        this.circleAngle = 0;
        this.circleRadius = 0;
        this.targetCircleRadius = Phaser.Math.Between(30, Math.min(sceneWidth * 0.1, sceneHeight * 0.1));
        
        // Store references to tweens and timers for cleanup
        this.movementTweens = [];
        this.movementTimers = [];
        
        // Single update function that combines all movement layers smoothly
        const updatePosition = () => {
            if (!this.sprite) return;
            
            // Calculate final position from base + offsets
            let finalX = this.baseX + Math.cos(this.circleAngle) * this.circleRadius;
            let finalY = this.baseY + Math.sin(this.circleAngle) * this.circleRadius + this.floatOffset;
        ...
        // Movement layer 1: Base position drift (main movement)
        const driftToNewPosition = () => {
            if (!this.sprite) return;
            
            // Calculate new target base position
            const moveDistanceX = Phaser.Math.Between(-sceneWidth * 0.12, sceneWidth * 0.12);
            const moveDistanceY = Phaser.Math.Between(-sceneHeight * 0.08, sceneHeight * 0.08);
            const targetBaseX = Phaser.Math.Clamp(this.baseX + moveDistanceX, minX + 50, maxX - 50);
            const targetBaseY = Phaser.Math.Clamp(this.baseY + moveDistanceY, minY + 50, maxY - 50);
            
            const distance = Math.sqrt(Math.pow(targetBaseX - this.baseX, 2) + Math.pow(targetBaseY - this.baseY, 2));
            const duration = Math.max(4000, Math.min(8000, distance * 20));
            
            const startBaseX = this.baseX;
            const startBaseY = this.baseY;
            
            const driftTween = this.scene.tweens.addCounter({
                from: 0,
                to: 1,
                duration: duration,
                ease: 'Sine.easeInOut',
                onUpdate: (tween) => {
                    ...
                    this.baseX = Phaser.Math.Linear(startBaseX, targetBaseX, progress);
                    this.baseY = Phaser.Math.Linear(startBaseY, targetBaseY, progress);
                    updatePosition();
                },
                onComplete: () => {
                    if (this.sprite) driftToNewPosition();
                }
            });
            this.movementTweens.push(driftTween);
        };
        
        // Movement layer 2: Gentle floating (vertical bob)
        const floatAnimation = () => {
            ...
            const floatTween = this.scene.tweens.addCounter({
                from: 0,
                to: Math.PI * 2,
                ...
                onUpdate: (tween) => {
                    if (!this.sprite) return;
                    this.floatOffset = Math.sin(tween.getValue()) * floatAmount;
                    updatePosition();
                }
            });
            this.movementTweens.push(floatTween);
        };
        
        // Movement layer 3: Subtle circular motion
        const circleAnimation = () => {
            if (!this.sprite) return;
            
            const circleDuration = 6000;
            
            const circleTween = this.scene.tweens.addCounter({
                from: 0,
                to: Math.PI * 2,
                ...
                onUpdate: (tween) => {
                    if (!this.sprite) return;
                    this.circleAngle = tween.getValue();
                    // Slowly interpolate radius toward target for smooth transitions
                    this.circleRadius = Phaser.Math.Linear(this.circleRadius, this.targetCircleRadius, 0.02);
                    updatePosition();
                }
            });
            this.movementTweens.push(circleTween);
            
            // Periodically change target radius for variety
            const radiusTimer = this.scene.time.addEvent({
                delay: 8000,
                callback: () => {
                    if (!this.sprite) return;
                    this.targetCircleRadius = Phaser.Math.Between(20, Math.min(sceneWidth * 0.1, sceneHeight * 0.1));
                },
                loop: true
            });
            this.movementTimers.push(radiusTimer);
        };
        
        // Start all movement layers (they modify different variables, combined in updatePosition)
        driftToNewPosition();
        floatAnimation();
        circleAnimation();
    }
    ...
    /**
     * Stop all movement tweens and timers
     */
    stopMovements() {
        if (this.movementTweens) {
            this.movementTweens.forEach(tween => {
        ...
        if (this.movementTimers) {
            this.movementTimers.forEach(timer => {
                if (timer) {
                    timer.remove();
                }
            });
