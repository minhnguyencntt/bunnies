/**
 * CountingForestScreen - Khu Rừng Đếm Số (Counting Forest)
...
class CountingForestScreen extends Phaser.Scene {
    ...
    preload() {
        // Load Counting Forest assets
        this.load.image('counting_forest_bg', 'screens/counting_forest/assets/backgrounds/l1_bg_1.png');
        this.load.audio('bgm_counting_forest', 'screens/counting_forest/assets/audio/bgm/level1_bgm.wav');
    ...
    create() {
        console.log('CountingForestScreen: create() called');
        ...
        console.log('CountingForestScreen: Initialized successfully');
    }
    ...
    createForestBackground() {
        ...
        // Use Counting Forest background image if available
        if (this.textures.exists('counting_forest_bg')) {
            const bg = this.add.image(width / 2, height / 2, 'counting_forest_bg');
            bg.setDisplaySize(width, height);
            bg.setDepth(0);
            console.log('Counting Forest background image displayed');
        } else {
            // Fallback: Create programmatic background if image not loaded
            console.warn('Counting Forest background image not found, using fallback graphics');
    ...
    playLevelBGM() {
        if (this.cache.audio.exists('bgm_counting_forest') && window.gameData?.musicEnabled !== false) {
            ...
            // Create and play Counting Forest BGM
            this.levelBGM = this.sound.add('bgm_counting_forest', {
                volume: 0.45, // Moderate volume for background ambiance
                loop: true
            });
            this.levelBGM.play();
            console.log('🎵 Playing Counting Forest BGM');
        }
    }
    ...
    showReward() {
        ...
        btn.on('pointerdown', () => {
            // Stop all sounds and return to menu
            this.sound.stopAll();
    ...
    showLevel2Transition(rewardBg, panel, rewardTitle, rewardSubtitle, star, btn, btnText) {
        const width = this.cameras.main.width;
        const height = this.cameras.main.height;
        
        // Fade out reward panel elements
        this.tweens.add({
            targets: [panel, rewardTitle, rewardSubtitle, star, btn, btnText],
            alpha: 0,
            duration: 500,
            onComplete: () => {
                if (panel) panel.destroy();
                if (rewardTitle) rewardTitle.destroy();
                if (rewardSubtitle) rewardSubtitle.destroy();
        ...
        this.time.delayedCall(600, () => {
            ...
            const transitionContainer = this.add.container(0, 0);
            transitionContainer.setDepth(260);

            // Get next level info for dynamic dialogue
            const nextLevelKey = typeof GameFlowConfig !== 'undefined' 
                ? GameFlowConfig.getNextScreen('CountingForestScreen') 
                : 'Level2Scene';
            const nextLevelInfo = typeof GameFlowConfig !== 'undefined' && nextLevelKey
                ? GameFlowConfig.getLevelInfo(nextLevelKey) 
                : { name: 'Thành Phố Gương', description: 'Tìm điểm khác biệt để khôi phục những tấm gương thiêng' };

            // Build dynamic transition dialogues
            let transitionDialogues;
            
            if (nextLevelKey === 'Level2Scene') {
                // Specific dialogue for Mirror City
                transitionDialogues = [
                    {
                        text: "Tuyệt vời! Cây cầu đã được sửa hoàn hảo. Những con số đã giúp bạn vượt qua thử thách đầu tiên!",
                        duration: 5000
                    },
                    {
                        text: "Bây giờ, hãy theo tôi đến Thành Phố Gương - nơi từng rực rỡ với ánh sáng phản chiếu từ ngàn tấm gương.",
                        duration: 5500
                    },
                    {
                        text: "Thế lực bóng tối đã phủ màn mờ lên 10 tấm gương thiêng. Chỉ có đôi mắt tinh tường của bạn mới có thể phá giải lời nguyền!",
                        duration: 6000
                    },
                    {
                        text: "Mỗi tấm gương ẩn chứa một bí ẩn - hãy tìm điểm khác biệt để khôi phục ánh sáng!",
                        duration: 5000
                    }
                ];
            } else if (nextLevelInfo) {
                // Generic dialogue for other levels
                transitionDialogues = [
                    {
                        text: "Tuyệt vời! Bạn đã hoàn thành xuất sắc thử thách này!",
                        duration: 4000
                    },
                    {
                        text: `Bây giờ, hãy theo tôi đến ${nextLevelInfo.name}!`,
                        duration: 4000
                    },
                    {
                        text: nextLevelInfo.description || "Một thử thách mới đang chờ đợi bạn!",
                        duration: 5000
                    }
                ];
            } else {
                // No next level - shouldn't happen but fallback
                transitionDialogues = [
                    {
                        text: "Tuyệt vời! Bạn đã hoàn thành tất cả các thử thách!",
                        duration: 4000
                    }
                ];
            }
            ...
            const showNextDialogue = () => {
                if (dialogueIndex >= transitionDialogues.length) {
                    // Create portal effect and transition to Level 2
                    this.createPortalTransition(transitionContainer, rewardBg);
                    return;
                }

                const dialogue = transitionDialogues[dialogueIndex];
                ...
                dialogueIndex++;
                this.time.delayedCall(dialogue.duration + 500, showNextDialogue);
            };
    ...
    createPortalTransition(transitionContainer, rewardBg) {
        const width = this.cameras.main.width;
        const height = this.cameras.main.height;
        ...
        // Portal circle with mirror-like shimmer
        const colors = [0xE1BEE7, 0xCE93D8, 0xBA68C8, 0xAB47BC, 0x9C27B0];
        
        let scale = 0;
        const animatePortal = this.time.addEvent({
            delay: 30,
            callback: () => {
                portal.clear();
                scale += 0.02;
                ...
                for (let i = 4; i >= 0; i--) {
                    ...
                    const radius = scale * 200 * (1 + i * 0.2);
                    portal.fillStyle(colors[i], alpha);
                    portal.fillCircle(0, 0, radius);
                }
                
                // Inner white glow
                portal.fillStyle(0xFFFFFF, 0.5 + Math.sin(scale * 10) * 0.2);
                portal.fillCircle(0, 0, scale * 100);
                
                if (scale >= 1.5) {
                    animatePortal.remove();
                    ...
                    this.time.delayedCall(500, () => {
                        ...
                        for (let i = 0; i < 30; i++) {
                            const sparkle = this.add.graphics();
                            sparkle.fillStyle(0xE1BEE7, 0.8);
                            sparkle.fillCircle(0, 0, 4);
                            sparkle.x = width / 2;
                            sparkle.y = height / 2;
                            sparkle.setDepth(280);
                            ...
                            this.tweens.add({
                                targets: sparkle,
                                x: width / 2 + Math.cos(angle * Math.PI / 180) * dist,
                                y: height / 2 + Math.sin(angle * Math.PI / 180) * dist,
                                alpha: 0,
                                scale: 0,
                                duration: 800,
                        ...
                        // Final fade and scene transition
                        this.time.delayedCall(1000, () => {
                            this.cameras.main.fadeOut(1000, 0, 0, 0);
                            
                            this.cameras.main.once('camerafadeoutcomplete', () => {
                                // Stop all sounds immediately (not with tween) before transitioning
                                this.sound.stopAll();
                                this.scene.stop('UIScene');
                                
                                // Use GameFlowConfig to determine next level
                                const nextLevel = typeof GameFlowConfig !== 'undefined' 
                                    ? GameFlowConfig.getNextScreen('CountingForestScreen') 
                                    : 'Level2Scene';
