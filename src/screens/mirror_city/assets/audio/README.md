...
audio/
├── bgm/
│   └── bgm.wav                 # Nhạc nền (generate_audio.py bgm)
└── voice/
...
File `bgm/bgm.wav` được tạo bằng `scripts/generate_audio.py bgm mirror_city_level2` (~90 giây).

**Để tạo lại:** `python3 scripts/generate_audio.py bgm mirror_city_level2`
...
### Cách 1: Tự động bằng Python (Khuyến nghị):

```bash
python3 scripts/generate_audio.py bgm mirror_city_level2
```

**Yêu cầu:** `pip install numpy scipy` (hoặc `pip install -r scripts/requirements_audio.txt`)
...
Các file audio được load trong `src/screens/mirror_city/screen.js`:
