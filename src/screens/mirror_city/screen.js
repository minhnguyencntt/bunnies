...
class MirrorCityScreen extends Phaser.Scene {
    ...
    createMirrorGallery() {
        ...
        const hudH = 80;
        const spacingX = Math.max(140, (width * 0.82) / 4);
        const spacingY = 155;
        const totalWidth = 4 * spacingX;
        const startX = (width - totalWidth) / 2;
        const startY = Math.max(hudH + mirrorHeight / 2 + 30, height * 0.32);
    ...
    showChallengeView() {
        ...
        const instructionText = this.add.text(width / 2, height - 80, 'Tìm điểm khác biệt giữa hai hình!', {
    ...
    createImagePanels(width, height, titleBottom) {
        ...
        const panelHeight = Math.min(availableHeight * 0.82, height * 0.48);
